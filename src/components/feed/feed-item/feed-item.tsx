...
import { OrderStatus, TFeedItem, TFeedOrder } from "../../types/feed";
...
export default function FeedItem({ order, type }: TFeedItem) {
   ...
   const orderStatus : string = order.status === "created" ? OrderStatus.created : order.status === "pending" ? OrderStatus.pending : OrderStatus.done

   return (
      <li>
         <Link to={type === "feed" ? `/feed/${order.number}` : `/profile/orders/${order.number}`} state={{ background: location }}>
            <button className={`${s.item} p-6`} onClick={() => handleClick()}>
               ...
               {type === "profile" && <p className={`text text_type_main-default pb-6 ${orderStatus === "Выполнен" && s.done_order}`}>{orderStatus}</p>}
               <div className={s.body}>
                  <ul className={s.ingredients}>
                     {orderIngredients.length <= 6 ? orderIngredients.map((orderIngredient, index) => (
                        <li key={index}>
                           <IngredientCircle index={orderIngredients.length - index} ingredient={ingredients.filter(ingredient => ingredient._id === orderIngredient)[0]} />
                        </li>
                     )) :
                     ...
                     {
                        orderIngredients.length > 5 && <li><IngredientCircle restIngredients={orderIngredients.length - 5} /></li>
                     }
                  </ul>
