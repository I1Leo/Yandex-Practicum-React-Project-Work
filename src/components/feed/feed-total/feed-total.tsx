...
export default function FeedTotal({ data }: Omit<TFeed, 'type'>): JSX.Element {
	...
	const doneOrders: Array<TFeedOrder> = orders
		? orders.filter((order: TFeedOrder) => order.status === 'done')
		: [];
	const pendingOrders: Array<TFeedOrder> = orders
		? orders.filter((order: TFeedOrder) => order.status === 'pending')
		: [];
		...
		<>
			<div>
						...
						<div className={`${s.list_container} pb-15`}>
							<ul className={s.list}>
								{doneOrders.slice(0, 5).map((doneOrder: TFeedOrder) => (
									<li
										key={doneOrder.number}
										className={`text text_type_digits-default ${s.list_done_item}`}>
										{doneOrder.number}
									</li>
								))}
							</ul>
				...
				<div className={'pb-15'}>
					...
					<p className={`text text_type_digits-large ${s.total_digits}`}>
						{data.total}
					</p>
				</div>
				<div>
