...
export default function OrdersInfo({
	isModal,
	type,
}: TOrdersInfo): JSX.Element | null {
	const { orderNumber } = useParams();

	const currentOrder = useAppSelector<TFeedOrder | null>((state) => {
		let order: TFeedOrder | undefined;

		if (type === 'feed' && orderNumber) {
			order = state.root.feed.feed?.orders.find(
				(o: TFeedOrder) => o.number === +orderNumber
			);
		...
		if (type === 'profile' && orderNumber) {
			order = state.root.profileFeed.orders?.orders.find(
				(o: TFeedOrder) => o.number === +orderNumber
			);
	...
	currentIngredients.forEach((currentIngredient: string) => {
		if (!(currentIngredient in currentIngredientsSet)) {
			const currentIngredientInfo: TIngredients | undefined = ingredients.find(
	...
	totalPrice = currentIngredients.reduce(
		(sum: number, currentIngredient: string) => {
			const ingredient: TIngredients | undefined = ingredients.find(
				(ingredient: TIngredients) => ingredient._id === currentIngredient
			);
			...
			{!isModal ? (
				<section>
					<div className={s.container}>
						<h3 className={`text text_type_digits-default pb-10 ${s.title}`}>
							#{currentOrder.number}
						</h3>
						<p className='text text_type_main-large pb-3'>
							{currentOrder.name}
						</p>
						<p
