...
export default function OrderPage(): JSX.Element | null {
	const orders = useAppSelector<TFeedOrder[] | undefined>(
		(state) => state.root.profileFeed.orders?.orders
	);
	...
	useEffect(() => {
		dispatch(
			wsProfileFeedConnect(
				`${ORDERS_SERVER_URL}?token=${localStorage
					.getItem('accessToken')
					?.slice(7)}`
			)
		);
				...
				<ul className={s.list}>
					{orders &&
						orders.map((order: TFeedOrder) => (
							<FeedItem type='profile' key={order.number} order={order} />
						))}
				</ul>
			) : (
				<Preload />
